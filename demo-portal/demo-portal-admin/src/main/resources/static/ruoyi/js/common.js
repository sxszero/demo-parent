...
var sub = {
    ...
    getColumnNum:function () {
        var data = $("#" + table.options.id).bootstrapTable('getData',{useCurrentPage:true,includeHiddenRows:true});
        return data.length;
    }
};
...
// 初始化所有日期
function initAllLaydate() {
    if ($(".time-input").length > 0) {
        $(".time-input").each(function (index, item) {
            initSingleLaydate(item);
        });
    }
}
// 刷新单个日期
function freshSingleLaydate(parentId,currId,html) {
    $("#"+currId).remove();
    $("#"+parentId).html(html);
    initSingleLaydate($("#"+currId).get(0));
}
// 初始化单个日期
function initSingleLaydate(item) {
    layui.use('laydate', function () {
